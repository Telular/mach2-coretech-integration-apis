...
namespace TankUtilityInterface
{
    ...
    public partial class frmTankUtilityInterface : Form, CommPkt.IObserver
    {
        public frmTankUtilityInterface()
        {
            ...
            TcpChannel channel = new TcpChannel(Convert.ToInt32(Properties.Settings.Default.RemotingTxPort));
                ...
                try
                {
                    ...
                    mobileOriginateSubClient.OnMessage(message =>
                    {
                        ...
                        try
                        {
                            ...
                            var messageStr = message.GetBody<String>();
                            sBody = message.GetBody<String>();
                            AddDebugLogItem(sBody);
                            tuMessage = JsonConvert.DeserializeObject<Payload>(sBody);
                        }
                        catch (Exception ex1)
                        {
                       ...
                       // cpMsg.sSourceNumber = sDeviceID;
                        ...
                        cpMsg.iSequence = 0;
                      //  cpMsg.dtUTCTimeStamp = tuMessage.UTCCreatedTimestamp;
