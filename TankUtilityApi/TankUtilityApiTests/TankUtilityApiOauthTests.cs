...
{
    [TestClass]
    public class TankUtilityApiOauthTests
    {
        readonly TankUtilityApi tankUtilityApi = new TankUtilityApi();       
        ...
        public async Task Test_PatchDeviceConfigAsync()
        {
            try
            {
                var deviceId = TankUtilityApiTestParams.GoodDeviceShortId;

                var response = await tankUtilityApi.GetDeviceAsync(deviceId);
                ...
                var newConfig = new Dictionary<string, object>
                {
                    { "address", $"{TankUtilityApiTestParams.GoodAddressForGAREVKMJ} {DateTime.UtcNow.ToString("o")}" }
                     // Used to set reading interval for TankUtilityApiTestParams.MikeReuterBlaineDevice
                     //{ "reading_interval", 10800}  
                };                

                var postResponse = await tankUtilityApi.PatchDeviceConfigAsync(deviceId, newConfig);
